...
[**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets) holds the listing behaviour, and [**ProjectNote**](data-model.md#projectnote--srcmodelstoreproject-notets) and [**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets) extend it. The markdown-level work — finding the section, matching an entry, rewriting one — is in `model/project/child-links.ts`.
...
**The write-loop guard.** A reconciler that both listens for a change and writes notes would otherwise see its own writes come back. The guard is the one [**BaseNote**](data-model.md#basenotefields-edit--srcmodelstorebase-notets) gives a field, carried one level down into the body:

- Every writer in `child-links.ts` hands back the listing it left, and [**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets)`.wrote` takes it onto the note's own reading, so a re-read landing exactly that wakes nobody.
- Nothing outside [**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets) calls those writers directly, [**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets)`.syncParentListing` included: it goes through the note that holds the line, not at the file.
...
- A note the plugin has just written is read back off the *file*, the metadata cache still holding what it said before. The read that would take it is a view's, so [**ProjectNoteStore**](data-model.md#projectnotestore--srcmodelstoreproject-note-storets)`.announce` takes it itself when the window closes on anything still owed — otherwise a note edited by hand while a write of the plugin's own was in the air would sit unreconciled until someone opened a tab.
...
[**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets)`.ensureListed` is the one case that adds a line. [**ProjectNoteStore**](data-model.md#projectnotestore--srcmodelstoreproject-note-storets) runs it only for a path **neither half has ever held** when the reparse reached it, **and** that is owed no read of the plugin's own — the second test being what keeps it off `createTask` and `moveTask`, which list the note themselves and would append the line twice if a second writer raced them. A listing that already names the arrival costs a cache read and no write.
...
[**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets)`.isVerified` is that knowledge — the standing `syncChildBoxes` decides on — and it is held for the session only: a note can change between two runs of the plugin, and a stored "verified" flag would be a claim about a file the plugin was not watching. A note leaving its path — a `delete`, or a `rename` seen under the old path — is dropped along with its flag, so whatever arrives there next is a note nobody has checked.
