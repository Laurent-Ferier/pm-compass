...
[**ListingIO**](data-model.md#listingiofields--srcmodeliolisting-iots) holds the listing behaviour, and [**ProjectIO**](data-model.md#projectio--srcmodelioproject-iots) and [**ProjectTaskIO**](data-model.md#projecttaskio--srcmodelioproject-task-iots) extend it. The markdown-level work — finding the section, matching an entry, rewriting one — is in `model/project/child-links.ts`.
...
- A **listing note** that changed is a listing that moved, so its boxes drive the tasks they name ([**ListingIO**](data-model.md#listingiofields--srcmodeliolisting-iots)`.applyChildBoxes` → [**ProjectTaskIO**](data-model.md#projecttaskio--srcmodelioproject-task-iots)`.applyParentBox`).
