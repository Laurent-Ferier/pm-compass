...
[**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets) (`model/store/listing-note.ts`) holds the listing behaviour, since a project and a parent task differ only in which section holds the list (`ChildLinkSection`) and where the children's own notes sit; [**ProjectNote**](data-model.md#projectnote--srcmodelstoreproject-notets) and [**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets) extend it and supply those two answers. It sits between them and [**BaseNote**](data-model.md#basenotefields-edit--srcmodelstorebase-notets) because a day note lists nothing — being a separate class is what says so, in place of every note having to answer "do I list children" with no. The markdown-level work — finding the section, matching an entry, rewriting one — is in `model/project/child-links.ts`.

## A listing is part of the note's reading

A note's fields are its frontmatter, plus this one thing that isn't: `NoteFields.listing` holds the boxes under its own heading, filled by [**NoteStore**](data-model.md#notestorefields-note-model--srcmodelstorenote-storets)`.parseNote` alongside everything else. So "the boxes moved" is a reading that moved, told apart from "the frontmatter moved" by the same `sameFields` comparison, and every rule below is answered from what the note holds rather than from a copy of the file passed around.
...
`syncChangedNote()` (`model/project/listing-sync.ts`) runs off the store's own telling that a note **changed** — [**ProjectNoteStore**](data-model.md#projectnotestore--srcmodelstoreproject-note-storets)`.announce`, at the end of the window a burst of vault events is gathered into. Obsidian's `changed` event is only what makes the store re-read the note; whether the reading moved is the models' answer, and a reparse that landed what the note already said reconciles nothing. The direction it syncs follows **which note it is**, not what changed inside it — diffing a listing against its previous self would decide nothing the rules below don't already decide:

- A **listing note** that changed is a listing that moved, so its boxes drive the tasks they name ([**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets)`.applyChildBoxes` → [**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets)`.applyParentBox`).
- A **task note** that changed is a status or title that moved, so it drives the line that lists it ([**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets)`.pushToListing` → `updateChildLink`).
...
**Where the listeners live.** Every handler belongs to [**ProjectNoteStore**](data-model.md#projectnotestore--srcmodelstoreproject-note-storets), which watches the folder from the moment the plugin loads, so the sync runs whether or not a tab is open — an edit made with every tab closed is answered like any other.

**The write-loop guard.** A reconciler that both listens for a change and writes notes would otherwise see its own writes come back. The guard is the one [**BaseNote**](data-model.md#basenotefields-edit--srcmodelstorebase-notets) gives a field — the note knows what it just wrote, so a re-read landing exactly that wakes nobody — carried one level down into the body: every writer in `child-links.ts` hands back the listing it left, and [**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets)`.wrote` takes it onto the note's own reading. Which is why nothing outside [**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets) calls those writers directly, [**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets)`.syncParentListing` included: it goes through the note that holds the line, not at the file. A task note's own `writeOwed` still pushes the title and the box to the listing directly as well as through the event: a write of the plugin's own is not worth waiting on Obsidian to reparse, and the listing has to move with an edit made while no view is open to hear it. A note the plugin has just written is read back off the *file* rather than off the metadata cache, which still holds what it said before — and since the read that would take it is a view's, [**ProjectNoteStore**](data-model.md#projectnotestore--srcmodelstoreproject-note-storets)`.announce` takes it itself when the window closes on anything still owed. Otherwise a note edited by hand while a write of the plugin's own was in the air would sit unreconciled until someone opened a tab.

**A note that arrives is not a note that changed.** `pushToListing` mirrors a task onto the line that lists it and adds none — `updateChildLink` leaves an absent entry absent, so a status pushed part-way through a move touches neither parent. That leaves the arrival: a task note the plugin never saw created, landing from a sync or an editor or a file copied in, which nothing lists and nothing therefore mirrors. [**ProjectTaskNote**](data-model.md#projecttasknote--srcmodelstoreproject-task-notets)`.ensureListed` is the one case that adds a line, and [**ProjectNoteStore**](data-model.md#projectnotestore--srcmodelstoreproject-note-storets) runs it only for a path **neither half has ever held** when the reparse reached it, and that is owed no read of the plugin's own — the second half of that test being what keeps it off `createTask` and `moveTask`, which list the note themselves and would append the line twice if a second writer raced them. A listing that already names the arrival costs a cache read and no write.

The opening pass is not the answer to this, and running it again on every arrival would be the wrong shape: it walks every live project, it is memoized on `verifyPass` to run once a session, and it is started from the warm-up — so a note Obsidian has yet to index when the plugin loads is one the pass's own walk doesn't hold, and it never runs again to notice. What an arrival needs is the one listing it belongs to.
...
[**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets)`.isVerified` is that knowledge: a flag on the note itself, saying its listing has been checked this session. A verified note has its boxes read as edits; one that isn't has its boxes answered by the statuses ([**ListingNote**](data-model.md#listingnotefields--srcmodelstorelisting-notets)`.repairChildBoxes`), and becomes verified by having been — which is what `syncChildBoxes` decides between. The flag is per-session and not persisted — a note can change between two runs of the plugin, and a stored "verified" flag would be a claim about a file the plugin was not watching. It is not part of the note's reading, so it stays out of `NoteFields` and out of `sameFields`: a note whose standing changed hasn't moved as far as a view is concerned. A note leaving its path — a `delete`, or a `rename` seen under the old path — is dropped along with its flag, so whatever arrives there next is a note nobody has checked.
