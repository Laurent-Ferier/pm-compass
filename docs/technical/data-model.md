...
- `uniquePathIn(app, folder, title, untitled, id, taken?)` — a free path for a note called `title`. The slug keeps every character a filesystem accepts, apostrophes and accents included, which is how obsidian-pm names its own notes: a name it doesn't read as the task's own it writes a second copy of. On collision the note takes the head of `id`, the one other name obsidian-pm reads that way. `untitled` names what the note *is* — `"task"`, `"project"` — for a title written in characters no slug survives. `taken` reserves paths not on disk yet, so a subtree moving whole allocates every destination up front and two moving siblings can't both claim one.
...
**ProjectTaskCache** is responsible for the task notes beside the projects, and the only maker of a [**ProjectTask**](#projecttask--srcmodelprojectproject-taskts) and a [**ProjectTaskIO**](#projecttaskio--srcmodelioproject-task-iots). It claims the notes [**ProjectCache**](#projectcache--srcmodelcacheproject-cachets) does not, and the two leave a folder together — a folder that has moved drops both readings at once, so neither half is ever read under a folder the other has left. Creating, updating and deleting a task note are [**ProjectService**](#projectservice--srcmodelserviceproject-servicets)'s: each writes a second note's listing too. It also resolves the fields a task points at another note with: obsidian-pm writes `projectId`, `parentId` and a dependency as `[[note|Title]]` where this plugin writes the id, and everything downstream matches ids, so the link is read as the id of the note Obsidian resolves it to.
