...
**Where the listeners live.** Every handler belongs to `ProjectNoteStore`, which watches the folder from the moment the plugin loads, so the sync runs whether or not a tab is open — an edit made with every tab closed is answered like any other.

**The write-loop guard.** A reconciler that both listens for a change and writes notes would otherwise see its own writes come back. The guard is the one `BaseNote` gives a field — the note knows what it just wrote, so a re-read landing exactly that wakes nobody — carried one level down into the body: every writer in `child-links.ts` hands back the listing it left, and `ListingNote.wrote` takes it onto the note's own reading. Which is why nothing outside `ListingNote` calls those writers directly, `ProjectTaskNote.syncParentListing` included: it goes through the note that holds the line, not at the file. `patchField` and `update()` still push to the listing directly as well as through the event: a write of the plugin's own is not worth waiting on Obsidian to reparse. A note the plugin has just written is read back off the *file* rather than off the metadata cache, which still holds what it said before — and since the read that would take it is a view's, `ProjectNoteStore.announce` takes it itself when the window closes on anything still owed. Otherwise a note edited by hand while a write of the plugin's own was in the air would sit unreconciled until someone opened a tab.

**A note that arrives is not a note that changed.** `pushToListing` mirrors a task onto the line that lists it and adds none — `updateChildLink` leaves an absent entry absent, so a status pushed part-way through a move touches neither parent. That leaves the arrival: a task note the plugin never saw created, landing from a sync or an editor or a file copied in, which nothing lists and nothing therefore mirrors. `ProjectTaskNote.ensureListed` is the one case that adds a line, and `ProjectNoteStore` runs it only for a path **neither half has ever held** when the reparse reached it, and that is owed no read of the plugin's own — the second half of that test being what keeps it off `createTask` and `moveTask`, which list the note themselves and would append the line twice if a second writer raced them. A listing that already names the arrival costs a cache read and no write.
...
The warm-up itself waits for `workspace.onLayoutReady`, and that wait is what makes the pass mean anything. A plugin's `onload` runs before Obsidian has finished listing the vault, so a folder walked there reads as however many files the tree happened to hold at that instant — measured on a phone: **three notes out of 123**, no projects among them. The pass is memoized to run once a session, so it vouched for those three and never ran again, and every listing in the vault stayed unrepaired until the user reached for the command. Watching still starts in `onload`: nothing that changes from that moment is missed, and the wait is only for the first read.
