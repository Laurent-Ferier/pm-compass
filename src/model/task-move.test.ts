...
function makeVault(overrides: Record<string, string> = {}) {
  return makeApp({
    ...
    [PATHS.kid]: taskFile({
      id: "kid", title: "Kid", parentId: "parent", type: "subtask",
      prefix: "Parent: [[parent|Parent]]", subtaskIds: ["grand"],
    }),
    [PATHS.grand]: taskFile({
      id: "grand", title: "Grand", parentId: "kid", type: "subtask", prefix: "Parent: [[kid|Kid]]",
    }),
...
describe("moveTask — same-project reparent", () => {
  ...
  it("keeps milestone type when moving to root", async () => {
    const app = makeVault({
      [PATHS.kid]: taskFile({
        id: "kid", title: "Kid", parentId: "parent", type: "milestone", prefix: "Parent: [[parent|Parent]]",
      }),
    });
    const t = tasks();
    await moveTask(app, { ...t.kid, type: "milestone" } as Task, ALPHA_DEST, all(), PROJECTS);

    expect(app._files.get(PATHS.kid)).toContain('type: "milestone"');
  });
...
describe("moveTask — dependencies", () => {
  ...
  it("preserves dependencies internal to the moving subtree", async () => {
    ...
    const app = makeVault({
      [PATHS.grand]: taskFile({
        id: "grand", title: "Grand", parentId: "kid", type: "subtask",
