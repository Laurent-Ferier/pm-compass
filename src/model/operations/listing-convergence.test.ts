...
describe("the box/status sync settles", () => {
  ...
  it("after a task is closed from the modal", async () => {
    ...
    await new ProjectTaskFile(l.app, T1).patchField("status", "done");
    await settle(l);
  ...
  it("after a task is renamed from the modal", async () => {
    ...
    await new ProjectTaskFile(l.app, T1).patchField("title", "Do it better");
    await settle(l);
  ...
  it("after a task is created", async () => {
    ...
    await ProjectTaskFile.create(l.app, {
      ...
      title: "Fresh", description: "", status: "todo", type: "task", priority: Priority.None,
