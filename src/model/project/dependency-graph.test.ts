...
describe("liftDependencies", () => {
  ...
  it("lifts a child's dependency onto its parent's card", () => {
    ...
    const tasks = tree().map((t) => (t.id === "a1" ? new Task({ ...t, dependencies: ["b"] }) : t));
  ...
  it("lifts through several levels, to the nearest visible ancestor", () => {
    const tasks = tree().map((t) => (t.id === "a1x" ? new Task({ ...t, dependencies: ["b1"] }) : t));
  ...
  it("drops a dependency whose two ends lift onto the same card", () => {
    const tasks = tree().map((t) => (t.id === "a1x" ? new Task({ ...t, dependencies: ["a2"] }) : t));
  ...
  it("drops a dependency on a task that isn't there at all", () => {
    const tasks = tree().map((t) => (t.id === "b" ? new Task({ ...t, dependencies: ["gone"] }) : t));
  ...
  it("merges the dependencies lifting onto one pair of cards", () => {
    const tasks = tree().map((t) => {
      if (t.id === "a1") return new Task({ ...t, dependencies: ["b"] });
      if (t.id === "a2") return new Task({ ...t, dependencies: ["b1"] });
      return t;
    });
  ...
  it("draws a pair solid when one of the dependencies behind it is the cards' own", () => {
    const tasks = tree().map((t) => {
      if (t.id === "a") return new Task({ ...t, dependencies: ["b"] });
      if (t.id === "a1") return new Task({ ...t, dependencies: ["b1"] });
      return t;
    });
  ...
  it("keeps the two directions between one pair of cards apart", () => {
    const tasks = tree().map((t) => {
      if (t.id === "a1") return new Task({ ...t, dependencies: ["b"] });
      if (t.id === "b1") return new Task({ ...t, dependencies: ["a"] });
      return t;
    });
