...
describe("backfillRecurringHabits", () => {
  ...
  it("creates a not-yet-existing daily notes folder exactly once, even though days are backfilled concurrently", async () => {
    // Regression test: each day's DayNoteService.ensureFile() independently checks/creates the
  ...
  it("counts a day as neither created nor changed when DayNoteService.ensureFile() fails to produce a note", async () => {
    // Templater is configured but fails to create the note (resolves without a path)
    // and no file shows up on disk either, so DayNoteService.ensureFile() returns null.
