...
import { Task, isStaleInboxItem, resolveHabitsTag } from "./task";
import { day } from "../__testing__/dates";
import { Priority } from "../base-task";

// ---------------------------------------------------------------------------
// Task.parse — non-task lines
// ---------------------------------------------------------------------------

describe("Task.parse", () => {
  describe("non-task lines", () => {
    it("returns null for a plain text line", () => {
      expect(Task.parse("Just a heading", 0)).toBeNull();
    });

    it("returns null for a blank line", () => {
      expect(Task.parse("", 0)).toBeNull();
    });

    it("returns null for a bullet without a checkbox", () => {
      expect(Task.parse("- plain bullet", 0)).toBeNull();
    });

    it("returns null for a heading line", () => {
      expect(Task.parse("## Daily", 0)).toBeNull();
    });
  });
  ...
  describe("checked state", () => {
    it("parses an unchecked task", () => {
      const t = Task.parse("- [ ] Buy milk", 0)!;
      expect(t.checked).toBe(false);
    });

    it("parses a lowercase-x checked task", () => {
      const t = Task.parse("- [x] Buy milk", 0)!;
  ...
  describe("title", () => {
    it("strips a ✅ completion date", () => {
      const t = Task.parse("- [x] Buy milk ✅ 2026-06-29", 0)!;
      expect(t.title).toBe("Buy milk");
    });

    it("strips a 📅 due date", () => {
      const t = Task.parse("- [ ] Buy milk 📅 2026-07-01", 0)!;
      expect(t.title).toBe("Buy milk");
    });

    it("strips a ⏳ scheduled date", () => {
      const t = Task.parse("- [ ] Buy milk ⏳ 2026-06-30", 0)!;
      expect(t.title).toBe("Buy milk");
    });

    it("strips a 🛫 start date", () => {
      const t = Task.parse("- [ ] Buy milk 🛫 2026-06-28", 0)!;
      expect(t.title).toBe("Buy milk");
    });

    it("strips a ➕ created date", () => {
      const t = Task.parse("- [ ] Buy milk ➕ 2026-06-01", 0)!;
      expect(t.title).toBe("Buy milk");
    });
  ...
  describe("date fields", () => {
    it("extracts createdAt from ➕", () => {
      expect(Task.parse("- [ ] Task ➕ 2026-06-01", 0)!.createdAt).toEqual(day("2026-06-01"));
    });

    it("extracts completedAt from ✅", () => {
      expect(Task.parse("- [x] Task ✅ 2026-06-29", 0)!.completedAt).toEqual(day("2026-06-29"));
    });

    it("extracts dueDate from 📅", () => {
      expect(Task.parse("- [ ] Task 📅 2026-07-15", 0)!.dueDate).toEqual(day("2026-07-15"));
    });

    it("extracts scheduledDate from ⏳", () => {
      expect(Task.parse("- [ ] Task ⏳ 2026-07-10", 0)!.scheduledDate).toEqual(day("2026-07-10"));
    });

    it("extracts startDate from 🛫", () => {
      expect(Task.parse("- [ ] Task 🛫 2026-07-05", 0)!.startDate).toEqual(day("2026-07-05"));
    });

    it("returns null for all date fields when absent", () => {
      const t = Task.parse("- [ ] Plain task", 0)!;
  ...
  describe("priority", () => {
    it.each([
      ...
      expect(Task.parse(`- [ ] Task ${emoji}`, 0)!.priority).toBe(name);
    });

    it("returns null when no priority emoji is present", () => {
      expect(Task.parse("- [ ] Plain task", 0)!.priority).toBeNull();
    });
  });
  ...
  describe("tags", () => {
    it("extracts a single hashtag", () => {
      expect(Task.parse("- [ ] Morning run #daily", 0)!.tags).toEqual(["#daily"]);
    });

    it("extracts multiple hashtags", () => {
      expect(Task.parse("- [ ] Task #daily #health", 0)!.tags).toEqual(["#daily", "#health"]);
    });

    it("returns an empty array when no tags are present", () => {
      expect(Task.parse("- [ ] Plain task", 0)!.tags).toEqual([]);
    });
    ...
    it("does not extract tags that were part of stripped metadata", () => {
      ...
      const t = Task.parse("- [x] Task #daily ✅ 2026-06-29", 0)!;
  ...
  describe("rawLine and lineIndex", () => {
    it("preserves the original raw line including metadata", () => {
      ...
      expect(Task.parse(raw, 3)!.rawLine).toBe(raw);
    });

    it("stores the provided lineIndex", () => {
      expect(Task.parse("- [ ] Task", 7)!.lineIndex).toBe(7);
    });
  });
});

// ---------------------------------------------------------------------------
// Task.toUncheckedLine
// ---------------------------------------------------------------------------

describe("Task.toUncheckedLine", () => {
  it("converts [x] to [ ]", () => {
    expect(Task.toUncheckedLine("- [x] Task")).toBe("- [ ] Task");
  });
  ...
  it("strips a ✅ date appended by the plugin", () => {
    expect(Task.toUncheckedLine("- [x] Task ✅ 2026-06-29")).toBe("- [ ] Task");
  });

  it("strips ✅ date including the leading space", () => {
    const result = Task.toUncheckedLine("- [x] Task ✅ 2026-06-29");
  ...
  it("leaves already-unchecked lines unchanged (no ✅)", () => {
    expect(Task.toUncheckedLine("- [ ] Task")).toBe("- [ ] Task");
  });
});
...
describe("Task.toCheckedLine", () => {
  it("converts [ ] to [x]", () => {
    expect(Task.toCheckedLine("- [ ] Task", day("2026-06-30"))).toBe("- [x] Task ✅ 2026-06-30");
  });

  it("appends the formatted date", () => {
    expect(Task.toCheckedLine("- [ ] Task", day("2099-12-31"))).toContain("✅ 2099-12-31");
  });
  ...
  it("preserves metadata already present in the line", () => {
    ...
    expect(Task.toCheckedLine(raw, day("2026-06-30"))).toBe("- [x] Review PR #work 📅 2026-07-05 ✅ 2026-06-30");
  });
});

describe("Task.withUpdatedTitle", () => {
  it("replaces the title, keeping the checkbox marker", () => {
    expect(Task.withUpdatedTitle("- [ ] Old title", "New title")).toBe("- [ ] New title");
  });

  it("preserves the checked state", () => {
    expect(Task.withUpdatedTitle("- [x] Old title", "New title")).toBe("- [x] New title");
  });
  ...
  it("preserves trailing emoji metadata (dates, priority)", () => {
    ...
    expect(Task.withUpdatedTitle(raw, "New title")).toBe("- [ ] New title 📅 2026-07-05 ➕ 2026-06-30");
  });
  ...
  it("keeps a tag when the caller includes it in the new title (tags are part of the editable title, unlike dates/priority)", () => {
    ...
    expect(Task.withUpdatedTitle(raw, "New title #work")).toBe("- [ ] New title #work 📅 2026-07-05");
  });

  it("preserves a ✅ completion date", () => {
    const raw = "- [x] Old title ✅ 2026-06-30";
    expect(Task.withUpdatedTitle(raw, "New title")).toBe("- [x] New title ✅ 2026-06-30");
  });

  it("returns rawLine unchanged when it isn't a checklist line", () => {
    expect(Task.withUpdatedTitle("Not a task", "New title")).toBe("Not a task");
  });

  it("returns the plain new title when there is no metadata to preserve", () => {
    expect(Task.withUpdatedTitle("- [ ] Old title", "Brand new")).toBe("- [ ] Brand new");
  });
});

describe("Task.withUpdatedScheduledDate", () => {
  const JULY_9 = new Date(2026, 6, 9);

  it("adds a ⏳ target date to a line that has none", () => {
    expect(Task.withUpdatedScheduledDate("- [ ] Alpha", JULY_9)).toBe("- [ ] Alpha ⏳ 2026-07-09");
  });

  it("replaces an existing target date rather than adding a second one", () => {
    expect(Task.withUpdatedScheduledDate("- [ ] Alpha ⏳ 2026-07-01", JULY_9))
      .toBe("- [ ] Alpha ⏳ 2026-07-09");
  });

  it("clears the target date when given null", () => {
    expect(Task.withUpdatedScheduledDate("- [ ] Alpha ⏳ 2026-07-09", null)).toBe("- [ ] Alpha");
  });

  it("leaves a line with no target date untouched when clearing", () => {
    const raw = "- [ ] Alpha  ➕ 2026-06-30   🔺";
    expect(Task.withUpdatedScheduledDate(raw, null)).toBe(raw);
  });

  it("keeps other metadata, with the target date after it", () => {
    expect(Task.withUpdatedScheduledDate("- [ ] Alpha 🔼 ➕ 2026-06-30", JULY_9))
      .toBe("- [ ] Alpha 🔼 ➕ 2026-06-30 ⏳ 2026-07-09");
  });

  it("preserves indentation, the checked state and tags", () => {
    expect(Task.withUpdatedScheduledDate("  - [x] Alpha #work ✅ 2026-06-30", JULY_9))
      .toBe("  - [x] Alpha #work ✅ 2026-06-30 ⏳ 2026-07-09");
  });

  it("round-trips through parse", () => {
    const line = Task.withUpdatedScheduledDate("- [ ] Alpha ➕ 2026-06-30", JULY_9);
    expect(Task.parse(line, 0)!.scheduledDate).toEqual(JULY_9);
    expect(Task.parse(line, 0)!.title).toBe("Alpha");
  });

  it("leaves a non-checkbox line alone", () => {
    expect(Task.withUpdatedScheduledDate("just text", JULY_9)).toBe("just text");
  });
});

describe("Task.withUpdatedPriority", () => {
  it("adds a priority marker to a line that has none", () => {
    expect(Task.withUpdatedPriority("- [ ] Alpha", Priority.High)).toBe("- [ ] Alpha ⏫");
  });

  it("replaces an existing priority marker", () => {
    expect(Task.withUpdatedPriority("- [ ] Alpha 🔽", Priority.Critical)).toBe("- [ ] Alpha 🔺");
  });

  it("clears the marker when given an empty priority", () => {
    expect(Task.withUpdatedPriority("- [ ] Alpha ⏫", Priority.None)).toBe("- [ ] Alpha");
  });

  it("keeps other metadata, with the marker ahead of it", () => {
    const raw = "- [ ] Alpha ➕ 2026-06-30";
    expect(Task.withUpdatedPriority(raw, Priority.Medium)).toBe("- [ ] Alpha 🔼 ➕ 2026-06-30");
  });

  it("moves a mid-title marker after the title rather than leaving it in place", () => {
    expect(Task.withUpdatedPriority("- [ ] Alpha ⏫ beta", Priority.Low)).toBe("- [ ] Alpha beta 🔽");
  });

  it("preserves indentation, the checked state and tags", () => {
    const raw = "  - [x] Alpha #work ✅ 2026-06-30";
    expect(Task.withUpdatedPriority(raw, Priority.High)).toBe("  - [x] Alpha #work ⏫ ✅ 2026-06-30");
  });

  it("round-trips through parse", () => {
    const line = Task.withUpdatedPriority("- [ ] Alpha ➕ 2026-06-30", Priority.Critical);
    expect(Task.parse(line, 0)!.priority).toBe(Priority.Critical);
    expect(Task.parse(line, 0)!.title).toBe("Alpha");
  });
  ...
  it("keeps a recurrence rule attached to its 🔁 marker instead of shedding it into the title", () => {
    ...
    expect(Task.withUpdatedPriority(raw, Priority.Medium))
  ...
  it("does not let a recurrence rule swallow a trailing tag", () => {
    ...
    expect(Task.withUpdatedPriority(raw, Priority.Low))
      .toBe("- [ ] Water plants #home 🔽 🔁 every week");
  });

  it("returns rawLine unchanged when it isn't a checklist line", () => {
    expect(Task.withUpdatedPriority("Not a task", Priority.High)).toBe("Not a task");
  });
});

describe("Task.parse — recurrence", () => {
  it("keeps the rule out of the title", () => {
    expect(Task.parse("- [ ] Water plants 🔁 every 2 weeks ➕ 2026-06-01", 0)!.title)
      .toBe("Water plants");
  });

  it("still reads tags that follow the rule", () => {
    expect(Task.parse("- [ ] Water plants 🔁 every week #home", 0)!.tags).toEqual(["#home"]);
  });
});
...
describe("Task.displayTitle", () => {
  it("strips the habits tag from the title", () => {
    expect(Task.parse("- [ ] Morning run #daily", 0)!.displayTitle("daily")).toBe("Morning run");
  });

  it("strips all tags from the title", () => {
    expect(Task.parse("- [ ] Task #daily #work", 0)!.displayTitle("daily")).toBe("Task");
  });

  it("strips a prefix-similar tag (#dailyish is still stripped as a tag)", () => {
    expect(Task.parse("- [ ] Read 30 min #dailyish", 0)!.displayTitle("daily")).toBe("Read 30 min");
  });

  it("returns title unchanged when habits tag is absent", () => {
    expect(Task.parse("- [ ] Plain task", 0)!.displayTitle("daily")).toBe("Plain task");
  });

  it("respects a non-default habits tag", () => {
    expect(Task.parse("- [ ] Meditate #habit", 0)!.displayTitle("habit")).toBe("Meditate");
  });
...
describe("Task.habitMatchTitle", () => {
  it("strips only the habits tag, unlike displayTitle it leaves other tags in place", () => {
    expect(Task.parse("- [ ] Read #book #daily", 0)!.habitMatchTitle("daily")).toBe("Read #book");
  });

  it("returns title unchanged when the habits tag is absent", () => {
    expect(Task.parse("- [ ] Plain task", 0)!.habitMatchTitle("daily")).toBe("Plain task");
  });
});
...
describe("Task.create", () => {
  it("creates an unchecked task with the given title and createdAt", () => {
    ...
    const t = Task.create("Buy milk", d);
    expect(t.title).toBe("Buy milk");
    expect(t.checked).toBe(false);
  ...
  it("builds the correct rawLine", () => {
    expect(Task.create("Buy milk", day("2026-07-01")).rawLine).toBe("- [ ] Buy milk ➕ 2026-07-01");
  });

  it("extracts tags from the title", () => {
    expect(Task.create("Morning run #daily", day("2026-07-01")).tags).toEqual(["#daily"]);
  });

  it("sets all other date and priority fields to null", () => {
    const t = Task.create("Task", day("2026-07-01"));
    expect(t.completedAt).toBeNull();
    expect(t.dueDate).toBeNull();
    expect(t.scheduledDate).toBeNull();
    expect(t.startDate).toBeNull();
...
describe("as a BaseTask", () => {
  it("is dated by its ⏳ target day", () => {
    expect(Task.parse("- [ ] Call the bank ⏳ 2026-07-09", 0)!.plannedDate).toEqual(day("2026-07-09"));
  });

  it("falls back to its 📅 deadline when nothing targets a day", () => {
    expect(Task.parse("- [ ] Call the bank 📅 2026-07-11", 0)!.plannedDate).toEqual(day("2026-07-11"));
  });

  it("prefers the target day to the deadline — that is the day it is waiting for", () => {
    expect(Task.parse("- [ ] Call ⏳ 2026-07-09 📅 2026-07-11", 0)!.plannedDate).toEqual(day("2026-07-09"));
  });

  it("has no date when the line carries none", () => {
    expect(Task.parse("- [ ] Call the bank", 0)!.plannedDate).toBeUndefined();
  });
});
