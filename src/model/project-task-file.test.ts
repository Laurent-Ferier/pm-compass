...
describe("ProjectTaskFile.patchField", () => {
  it("throws when the file does not exist", async () => {
    const app = makeApp();
    await expect(new ProjectTaskFile(app, TASK_PATH).patchField("status", "done")).rejects.toThrow("File not found");
  });
  ...
  it("sets the priority field", async () => {
    const app = makeApp({ [TASK_PATH]: makeTaskContent() });
    await new ProjectTaskFile(app, TASK_PATH).patchField("priority", "critical");
